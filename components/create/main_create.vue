<template>
  <v-row v-if="!selectedMission">
    <v-col v-if="!isMobile" cols="12" sm="12" md="2">
      <v-row>
        <v-col cols="12" class="justify-center align-center d-flex pa-1">
          <v-card-title class="text-h6" style="font-weight: bold">Create Mission</v-card-title>
        </v-col>

        <v-col cols="12" class="justify-center d-flex pa-1 px-4">
          <v-btn rounded="xl" size="large" block color="#2A3547" @click="submitForm">
            <span style="text-transform: none" class="text-h6">Create new</span>
          </v-btn>

          <v-divider vertical :thickness="2" style="margin-top: -55px" class="px-3 border-opacity-50"
            color="#707070"></v-divider>
        </v-col>
      </v-row>
    </v-col>
    ...
    <v-col v-else cols="12" sm="12" md="2">
      <v-row>
        <v-col cols="12" class="justify-center align-center d-flex pa-1">
          <v-card-title class="text-h5" style="font-weight: bold">Create Mission</v-card-title>
        </v-col>

        <v-col cols="12" class="justify-center d-flex pa-1 px-4">
          <v-btn rounded="xl" size="large" block color="#2A3547" @click="submitForm">
            <span style="font-size: 16px; text-transform: none">Create new</span>
          </v-btn>
        </v-col>
      </v-row>
    </v-col>
    ...
    <v-col cols="12" sm="12" md="10">
      <v-row>
        <v-col cols="12" sm="12" md="3" class="pa-4 align-center justify-center">
          <v-col cols="12" class="d-flex justify-center">
            <vue-dropzone ref="myVueDropzone" id="dropzone" :options="dropzoneOptions" class="custom-dropzone" />
          </v-col>
          <v-col cols="12" class="d-flex justify-center">
        ...
        <v-col cols="12" sm="12" md="8">
          <v-card-text>
            <v-form ref="formRef" v-model="valid">
              ...
              ></v-select> -->
              <v-select density="compact" label="Select mission" variant="outlined" rounded="lg" style="margin-top: 5px"
                :items="missions" item-value="mission" v-model="selectedMission">
              ...
              <v-row class="mt-4">
                <v-col cols="12">
                  <div>
                    <v-chip v-for="(message, index) in formData.messages" :key="index" class="ma-1" close
  ...
  <v-row v-else>
    ...
    <v-col cols="12" sm="12" md="10">
      <component :is="getMissionComponent(selectedMission)" :teams="teams" :selectedMission="selectedMission"
