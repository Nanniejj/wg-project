<template>
  <div>
    ...
    <v-select clearable label="Select Mission" v-model="mission" :items="formattedItems" item-title="label"
    ...
    <MissionM4 v-if="mission === 'M4'" />
    <MissionM5 v-if="mission === 'M5'" />
    <MissionM6 v-if="mission === 'M6'" />
    ...
    <MissionR10 v-if="mission === 'R10'" />
    <MissionR11 v-if="mission === 'R11'" />
    <MissionR12 v-if="mission === 'R12'" />
  </div>
</template>
...
export default {
  data() {
    return {
      mission: "M4", // Default selected mission
      items: [
        "M1",
        "M2",
        "M3",
  ...
  computed: {
    // Map items to a formatted structure
    formattedItems() {
