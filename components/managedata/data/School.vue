<template>
  <div class="text-end pb-10">
    <div v-if="!isMobile">
      <v-row class="justify-end">
        <div class="pa-1">
          <v-btn color="#AEE0E8" outlined rounded="lg" @click="triggerFileInput">
            <v-icon class="px-3">mdi-tray-arrow-down</v-icon><span>Import CSV</span>
          </v-btn>

          <input ref="fileInput" type="file" accept=".csv" @change="handleFileUpload" style="display: none" c />
        </div>
        <div class="pa-1">
          <v-btn color="#E58383" rounded="lg" @click="dialogSchool()"><v-icon color="white">mdi-plus</v-icon>
    ...
    <div v-else>
      ...
      <v-row class="justify-end">
        <div class="pa-1">
          <v-btn color="#AEE0E8" size="large" density="comfortable" icon="mdi-tray-arrow-down"
            @click="triggerFileInput"></v-btn>

          <input ref="fileInput" type="file" accept=".csv" @change="handleFileUpload" style="display: none" />
        </div>
        <div class="pa-1">
          <v-btn color="#E58383" size="large" density="comfortable" icon="mdi-plus" @click="addSchool = true"></v-btn>
        </div>
      </v-row>
    </div>
  </div>
  ...
  <div class="pb-8">
    <v-text-field v-model="search" placeholder="ค้นหาชื่อโรงเรียน" prepend-inner-icon="mdi-magnify" hide-details
      rounded="lg" variant="solo">
  ...
  <v-row>
    <v-col cols="12" md="3" class="py-0">
      ...
      <v-autocomplete v-model="selectedZoneId" :items="zoneItems" item-title="name" item-value="id" density="compact"
        placeholder="เลือกภาค" variant="solo" closable-chips @update:modelValue="updateProvinces" dense hide-no-data />
    </v-col>
    <v-col cols="12" md="3" class="py-0">
      <v-autocomplete v-model="selectedProvinceId" :items="ProvinceItems" item-title="name_th" item-value="id"
        density="compact" placeholder="เลือกจังหวัด" variant="solo" closable-chips @update:modelValue="updateDistrict"
        dense hide-no-data />
    </v-col>
    <v-col cols="12" md="3" class="py-0">
      <v-autocomplete v-model="selectedDistrictId" :items="districtItems" item-title="name_th" item-value="id"
        density="compact" placeholder="เลือกอำเภอ" variant="solo" closable-chips @update:modelValue="updateSubDistrict"
        dense hide-no-data />
    </v-col>
    <v-col cols="12" md="2" class="py-0">
      <v-autocomplete v-model="filterLevel" :items="level" item-title="text" item-value="value" density="compact"
        variant="solo" placeholder="ระดับปฏิบัติการ"></v-autocomplete>
    </v-col>
  ...
  <div class="pt-5">
    <v-data-table :headers="headers" :items="school_value" :mobile="isMobile" :hide-default-header="isMobile"
      hide-default-footer :loading="isLoading" loading-text="กำลังโหลดข้อมูล...">
      ...
      <template v-slot:item.POC="{ item }">
        ...
        <div :style="isMobile ? 'margin-left: 0; margin-right: 0;' : 'margin-left: 0px;'
          ">


          <template v-if="item.stakeholder && Array.isArray(item.stakeholder) && item.stakeholder.length > 0">
            ...
            <template v-if="item.stakeholder.length > 1">
              ...
              <div style="position: relative; width: 60px; height: 40px;">
                <!-- วงกลมแรก -->
                <v-avatar size="40" style="position: absolute; top: 0; left: 0; z-index: 2;">
                  <v-img :src="item.stakeholder[0].image" alt="Stakeholder Image" />
                </v-avatar>

                <!-- วงกลมที่สอง (ตัวเลข) -->
                <v-avatar size="30" color="grey" style="position: absolute; top: 5px; left: 35px; z-index: 1;">
                  <span
                    style="font-size: 0.8em; display: flex; align-items: center; justify-content: center; color: white;">
            ...
            <template v-else>
              ...
              <v-avatar size="40">
                <v-img :src="item.stakeholder[0].image" alt="Stakeholder Image"></v-img>
              </v-avatar>
            </template>
          </template>
      ...
      <template v-slot:item.actions="{ item }">
        <div v-if="!isMobile" class="align-items-center d-flex">
          <v-icon v-tooltip="'Edit'" icon="mdi-pencil" @click="
            itemToAction = item;
          EditOverlay = true;
          "></v-icon>
          <v-icon v-tooltip="'Delete'" icon="mdi-trash-can" @click="
            itemToAction = item;
          deleteOverlay = true;
          "></v-icon>
        </div>
        <div v-else class="align-items-center">
          <span style="color: #29a0af" @click="
            itemToAction = item;
          EditOverlay = true;
          ">แก้ไข</span>
          <span style="color: red; margin-left: 35px" @click="
            itemToAction = item;
          deleteOverlay = true;
          ">ลบ</span>
        </div>
  ...
  <div class="text-center pt-16">
    <v-pagination v-model="page" :length="pagination.totalPages" rounded="circle" class="my-4"></v-pagination>
  </div>
  ...
  <v-dialog v-model="addSchool" max-width="1200">
    <v-card class="pa-2" rounded="lg">
      <v-card-title class="text-h6 d-flex justify-space-between align-center">
        ...
        <v-btn icon="mdi-close" variant="text" size="large" @click="addSchool = false"></v-btn>
      </v-card-title>
      <div class="pt-1 pb-1">
      ...
      <v-card-text class="pt-4">
        <v-row>
          <v-col cols="12" sm="8" class="pt-0 pb-0">
            ...
            <v-text-field v-model="SchoolValue" placeholder="เพิ่มชื่อโรงเรียน" variant="outlined" density="compact"
          ...
          <v-col cols="12" sm="4" class="pt-0 pb-0">
            ...
            <!-- Dropdown สำหรับเลือกภาค -->
            <v-autocomplete v-model="selectedZoneId" :items="zoneItems" item-title="name" item-value="id"
              density="compact" placeholder="ภาค" variant="outlined" closable-chips @update:modelValue="updateProvinces"
              dense hide-no-data />
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12" sm="6" class="pt-0 pb-0">
            ...
            <!-- Dropdown สำหรับเลือกภาค -->
            <v-autocomplete v-model="selectedProvinceId" :items="ProvinceItems" item-title="name_th" item-value="id"
              density="compact" placeholder="จังหวัด" variant="outlined" closable-chips
              @update:modelValue="updateDistrict" dense hide-no-data />
          </v-col>
          <v-col cols="12" sm="6" class="pt-0 pb-0">
            <span class="text-h6">เขต/อำเภอ</span>
            <!-- Dropdown สำหรับเลือกภาค -->
            <v-autocomplete v-model="selectedDistrictId" :items="districtItems" item-title="name_th" item-value="id"
              density="compact" placeholder="เขต/อำเภอ" variant="outlined" closable-chips
              @update:modelValue="updateSubDistrict" dense hide-no-data />
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12" sm="8" class="pt-0 pb-0">
            ...
            <!-- Dropdown สำหรับเลือกภาค -->
            <v-autocomplete v-model="selectedSubDistrictId" :items="SubDistrictItems" item-title="name_th"
              item-value="id" density="compact" placeholder="แขวง/ตำบล" variant="outlined" closable-chips dense
              hide-no-data />
          </v-col>
          <v-col cols="12" sm="4" class="pt-0 pb-0">
            <span class="text-h6">ไปรษณีย์</span>
            <v-text-field :items="SubDistrictItems" item-title="zip_code" item-value="zip_code" return-object
              density="compact" variant="outlined" placeholder="รหัสไปรษณีย์" :rules="[isNumber]"
              :model-value="initialZipCode"></v-text-field>
          </v-col>
        </v-row>
        ...
        <v-row>
          <v-col cols="12" md="4" class="py-0">
            ...
            <v-number-input density="compact" v-model="NumStudent" :min="0" control-variant="split" inset
              variant="outlined"></v-number-input>
          </v-col>
          <v-col cols="12" md="4" class="py-0">
            <span class="text-h6">จำนวนแกนนำ</span>
            <v-number-input density="compact" v-model="NumLead" :min="0" control-variant="split" inset
              variant="outlined"></v-number-input>
          </v-col>

          <v-col cols="12" md="4" class="py-0">
            <span class="text-h6">เลือกระดับปฏิบัติการ</span>
            <v-autocomplete v-model="selectedLevel" :items="level" item-title="text" item-value="value"
              density="compact" variant="outlined" label="เลือกระดับ"></v-autocomplete>
          </v-col>
        </v-row>
        <!-- <v-row>
          <v-col cols="12" md="4">
        ...
        <div class="justify-end d-flex pt-10">
          <v-btn @click="addDropzone" color="#AEE0E8" elevation="2" rounded class="mx-2" style="font-size: 16px">
            <v-icon left>mdi-account-plus</v-icon> เพิ่ม POC
          </v-btn>
        </div>
        <div v-for="(dropzone, index) in dropzones" :key="index" class="dropzone-container pt-10">
          <v-row>
            <v-col cols="12" md="2" class="justify-center d-flex">
              <!-- Dropzone Component -->
              <vue-dropzone :id="'dropzone-' + index" :options="dropzoneOptions" class="custom-dropzone"
                @vdropzone-success="handleSuccess(index)"
                @vdropzone-file-added="handleFileAdded(index, $event)" /></v-col>
            <v-col cols="12" md="10">
              <span class="text-h6">POC ({{ index + 1 }})</span>
              <!-- Input สำหรับกรอกชื่อภาพ -->
              <v-text-field type="text" class="w-100" :id="'imageName-' + index" v-model="dropzone.imageName"
                placeholder="เพิ่มชื่อผู้ที่เกี่ยวข้อง" variant="outlined" density="compact" rounded="lg" />
            </v-col>
          </v-row>
        </div>
      </v-card-text>
      ...
      <div class="d-flex justify-end pb-6 px-6">
        <v-btn color="#2A3547" rounded="lg" size="large" @click="onClick()" class="text-white" min-width="200">
          บันทึก
        </v-btn>
      </div>
    </v-card>
  ...
  <v-dialog v-model="EditOverlay" max-width="1200">
    <v-card class="pa-2" rounded="lg">
      <v-card-title class="text-h6 d-flex justify-space-between align-center">
        ...
        <v-btn icon="mdi-close" variant="text" size="large" @click="EditOverlay = false"></v-btn>
      </v-card-title>
  ...
  <v-dialog v-model="deleteOverlay" max-width="400">
    <v-card rounded="xl" elevation="7">
      ...
      <v-card-actions class="justify-center pt-5">
        <v-btn color="green" @click="navigateToDelete(itemToAction)" large>ยืนยัน</v-btn>
...
const onClick = async () => {
  ...
  //   // แนบข้อมูลแต่ละ stakeholder
  //   formData.append(`stakeholder[${index}][person]`, dropzone.imageName);

  //   if (file) {
  //     formData.append(`stakeholder[${index}][image]`, file);
  //   }
  //   console.log(`อัปโหลดสำหรับ POC ${index + 1} สำเร็จ: ${personName} ${file?.name}`);
  // });
  dropzones.value.forEach((dropzone, index) => {
